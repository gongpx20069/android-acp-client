...
bridge 需要 Python 3.11 或更高版本，启动时不会自行创建 Python 环境或安装 package。请在 `bridge` 目录中选择下面一种方式显式安装。
...
如需安装全部可选后端，请将 `requirements.txt` 替换为 `requirements-all.txt`。安装后使用下面的 `android-acp-bridge` 命令启动。源码辅助入口 `python .\run.py ...` 只会使用当前 Python 环境转发到同一个 CLI，不会创建环境或安装 package。

依赖分组和开发环境的更多说明见 [`bridge/README.md`](bridge/README.md)。
